...
export function OnboardingFlow({ selectedDate }: Props) {
  ...
  const isScreen2Valid = answers.activities.some(
    (a) => a.name.trim() !== "" && a.place.trim() !== "" && a.startTime.trim() !== "" && a.endTime.trim() !== ""
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const life_variables = {
        ...
        breakfast: { time: answers.breakfast.time, location: answers.breakfast.location },
      ...
      // Automatically generate the routine now that constraints are saved
      ...
      if (!res.ok) {
        throw new Error(data.error || "Failed to generate schedule from your constraints.");
      }

      router.push(
        selectedDate ? `/dashboard?date=${selectedDate}` : "/dashboard",
      );
    } catch (err: any) {
      setSaveError(err.message || "An error occurred while saving your routine.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const addActivity = () => {
    setAnswers((prev) => ({
      ...prev,
      activities: [...prev.activities, { name: "", place: "", startTime: "", endTime: "" }],
    }));
  };
  ...
  const updateMeal = (meal: "breakfast" | "lunch" | "dinner", field: "time" | "location", value: string) => {
    setAnswers((prev) => ({
      ...prev,
    ...
    <div className="mx-auto w-full max-w-md rounded-2xl border border-blue-900/30 bg-slate-900/40 p-6 text-white shadow-2xl backdrop-blur-xl">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="space-y-4">
            {answers.activities.map((activity, idx) => (
              <div key={idx} className="relative rounded-xl border border-blue-900/40 bg-slate-950/40 p-4">
      ...
      {currentScreen === 3 && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-blue-900/40 bg-slate-950/40 p-4">
              <h3 className="mb-3 font-medium text-blue-100">🍳 Breakfast</h3>
              <div className="grid gap-3 sm:grid-cols-2">
                <input
                  type="time"
                  value={answers.breakfast.time}
                  onChange={(e) => updateMeal("breakfast", "time", e.target.value)}
                  className="w-full rounded-xl border border-blue-900/50 bg-slate-950/50 px-3 py-2 text-white transition-all focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                />
                <input
                  type="text"
                  placeholder="e.g. Home, Campus canteen"
                  value={answers.breakfast.location}
                  onChange={(e) => updateMeal("breakfast", "location", e.target.value)}
                  className="w-full rounded-xl border border-blue-900/50 bg-slate-950/50 px-3 py-2 text-white placeholder-blue-300/30 transition-all focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                />
              </div>
            </div>
            ...
            <div className="rounded-xl border border-blue-900/40 bg-slate-950/40 p-4">
              <h3 className="mb-3 font-medium text-blue-100">🥗 Lunch</h3>
              <div className="grid gap-3 sm:grid-cols-2">
                <input
                  type="time"
                  value={answers.lunch.time}
                  onChange={(e) => updateMeal("lunch", "time", e.target.value)}
                  className="w-full rounded-xl border border-blue-900/50 bg-slate-950/50 px-3 py-2 text-white transition-all focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                />
                <input
                  type="text"
                  placeholder="Where?"
                  value={answers.lunch.location}
                  onChange={(e) => updateMeal("lunch", "location", e.target.value)}
                  className="w-full rounded-xl border border-blue-900/50 bg-slate-950/50 px-3 py-2 text-white placeholder-blue-300/30 transition-all focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                />
              </div>
            </div>
            ...
            <div className="rounded-xl border border-blue-900/40 bg-slate-950/40 p-4">
              <h3 className="mb-3 font-medium text-blue-100">🍽️ Dinner</h3>
              <div className="grid gap-3 sm:grid-cols-2">
                <input
                  type="time"
                  value={answers.dinner.time}
                  onChange={(e) => updateMeal("dinner", "time", e.target.value)}
                  className="w-full rounded-xl border border-blue-900/50 bg-slate-950/50 px-3 py-2 text-white transition-all focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                />
                <input
                  type="text"
                  placeholder="Where?"
                  value={answers.dinner.location}
                  onChange={(e) => updateMeal("dinner", "location", e.target.value)}
                  className="w-full rounded-xl border border-blue-900/50 bg-slate-950/50 px-3 py-2 text-white placeholder-blue-300/30 transition-all focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                />
              </div>
            </div>
